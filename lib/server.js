...
if (init) {
  nano.db.destroy('dbTransactions', function (err, body, header) {
    if (err) {
      console.log('Non-critical error. No tx database');
    }
    nano.db.destroy('dbAddresses', function (err, body, header) {
      if (err) {
        console.log('Non-critical error. No addr database');
      }
  ...
  constructor(address, trdAmount, txids) {
    ...
    if (txids) {
      ...
      for (let code of TOKEN_CODES) {
        this.amounts[code] = biggystring.bns.mul(trdAmount, n.toString());
        n++;
      }
    }
  }
...
function addBlockHeightToTransaction(txObj) {
  let txHeight = biggystring.bns.add(getBlockHeight(txObj.txDate), '3');

  const now = Date.now() / 1000;
  const blockHeight = getBlockHeight(now);

  const heightDiff = biggystring.bns.sub(blockHeight, txHeight);
  if (biggystring.bns.lt(heightDiff, '0')) {
    txHeight = '0';
  }
...
router.post('/spend', function (req, res) {
  ...
  for (const input of inputs) {
    if (biggystring.bns.lt(input.amount, 0)) {
    ...
    totalInputs[currencyCode] = biggystring.bns.add(totalInputs[currencyCode], input.amount);
  ...
  for (let output of outputs) {
    if (biggystring.bns.lt(output.amount, 0)) {
    ...
    totalOutputs[currencyCode] = biggystring.bns.add(totalOutputs[currencyCode], output.amount);
  ...
  for (let currencyCode of currencyCodes) {
    if (biggystring.bns.gt(totalOutputs[currencyCode], totalInputs[currencyCode])) {
  ...
  const networkFee = biggystring.bns.sub(totalInputs[PRIMARY_CURRENCY], totalOutputs[PRIMARY_CURRENCY]);
  if (biggystring.bns.lt(networkFee, MINIMUM_NETWORK_FEE)) {
...
function createAddress(addr, cb) {
  ...
  let txids = null;
  if (biggystring.bns.gt(nativeAmount, '0')) {
    ...
    for (let currencyCode of TOKEN_CODES) {
      const amt = biggystring.bns.mul(nativeAmount, n.toString());
  ...
  dbAddresses.insert(addressObj, addr, function (err, res) {
    if (err) {
      cb(err);
    } else if (biggystring.bns.gt(nativeAmount, '0')) {
  ...
  if (inputs.length === 0) {
    ...
    dbAddresses.get(input.address, function (err, addressObj) {
      if (err) {
        ...
        if (biggystring.bns.gte(addressObj.amounts[input.currencyCode], input.amount)) {
          inputs.splice(0, 1);
